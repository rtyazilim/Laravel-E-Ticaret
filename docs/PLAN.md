...
- FTP deployment işlemi sırasındaki hassas bağlantı bilgileri (sunucu, şifre, kullanıcı) ve hedef dizin bilgisi `deploy.yml` üzerinden tamamen kaldırılarak doğrudan GitHub Secrets (`FTP_SERVER`, `FTP_USERNAME`, `FTP_PASSWORD`, `FTP_REMOTE_DIR`) ile dinamik hale getirildi.
